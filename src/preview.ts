...
export function buildPreviewHtml(
  ...
  // Derive key colors from the array
  const primary = analysis.colors.find(c => /primary/i.test(c.name))?.hex ?? analysis.colors[0]?.hex ?? "#333333";
  const bg = analysis.colors.find(c => /background|neutral|surface/i.test(c.name))?.hex ?? "#ffffff";
  const surface = analysis.colors.find(c => /surface|card/i.test(c.name))?.hex ?? bg;
  const textColor = analysis.colors.find(c => /text|foreground|on-surface/i.test(c.name))?.hex ?? onColor(bg);
  const textMuted = analysis.colors.find(c => /secondary.*text|muted|subtle/i.test(c.name))?.hex ?? "#888888";
...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .overview{font-size:17px;line-height:1.7;color:${textMuted};max-width:720px}
  .section-text{font-size:15px;line-height:1.7;color:${textMuted};max-width:720px;white-space:pre-line}
...
<section>
  ...
  <div class="section-text">${analysis.typography}</div>
</section>
...
<section>
  ...
  <div class="section-text">${analysis.components}</div>
</section>
